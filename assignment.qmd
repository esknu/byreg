...
datapt1 <- datapt1 %>%
  ...
  mutate(Relative_Growth = Population / Population[Year == 2005]* 100/100) %>%
  ungroup()
...
ggplot(aes(x = Year, y = Relative_Growth, color = Region_Type)) +
  ...
  xlim(2005, 2022) +
  scale_y_continuous(breaks = seq(1, max(datapt1$Relative_Growth), by = 1))
```
...
housingcost <- housingcost |> 
  select(Year = TIME_PERIOD, Country = geo, Region_Type = deg_urb, dHousing = OBS_VALUE) |> 
...
```{r}
employmentrate <- employmentrate |> 
  select(Year = TIME_PERIOD, Country = geo, Region_Type = deg_urb, dEmployment = OBS_VALUE) |> 
  mutate(Region_Type = recode(Region_Type,
                           "DEG1" = "City",
                           "DEG2" = "Suburb",
                           "DEG3" = "Rural"))
...
unemploymentrate <- unemploymentrate |> 
  select(Year = TIME_PERIOD, Country = geo, Region_Type = deg_urb, dUnemployed = OBS_VALUE) |> 
  mutate(Region_Type = recode(Region_Type,
                           "DEG1" = "City",
                           "DEG2" = "Suburb",
                           "DEG3" = "Rural"))
...
```{r}
merged <- merged %>%
  merge(housingcost, by = c("Country", "Year", "Region_Type"), na.rm = TRUE) %>%
  filter(complete.cases(.))
```
...
ggplot(region_counts, aes(x = Class, y = Count, fill = Region_Type)) +
