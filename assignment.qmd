...
housingcost <- housingcost |> 
  select(Year = TIME_PERIOD, Country = geo, "Region Type" = deg_urb, housingprices = OBS_VALUE) |> 
...
```{r}
employmentrate <- employmentrate |> 
  select(Year = TIME_PERIOD, Country = geo, "Region Type" = deg_urb, employmentrate = OBS_VALUE) |> 
  mutate(`Region Type` = recode(`Region Type`,
                           "DEG1" = "City",
                           "DEG2" = "Suburb",
                           "DEG3" = "Rural"))
...
unemploymentrate <- unemploymentrate |> 
  select(Year = TIME_PERIOD, Country = geo, "Region Type" = deg_urb, unemploymentrate = OBS_VALUE) |> 
  mutate(`Region Type` = recode(`Region Type`,
                           "DEG1" = "City",
                           "DEG2" = "Suburb",
                           "DEG3" = "Rural"))
...
merged <- merged |> 
  ...
  mutate(meanUnemployment = mean(unemploymentrate)) |>
  ungroup()

merged <- merged |> 
  group_by(Country, Year) |> 
  mutate(meanEmploymentrate = mean(employmentrate)) |>
  ungroup()

merged <- merged |> 
  group_by(Country, Year) |> 
  mutate(meanHousingprices = mean(housingprices)) |>
  ungroup()
...
ggplot(region_counts, aes(x = Class, y = Count, fill = RegionType)) +
