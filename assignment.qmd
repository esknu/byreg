...
housingcost <- housingcost |> 
  select(Year = TIME_PERIOD, Country = geo, Class = deg_urb, dHousing = OBS_VALUE) |> 
...
```{r}
employmentrate <- employmentrate |> 
  select(Year = TIME_PERIOD, Country = geo, Class = deg_urb, dEmployment = OBS_VALUE) |> 
  mutate(Class = recode(Class,
                           "DEG1" = "City",
                           "DEG2" = "Suburb",
                           "DEG3" = "Rural"))
...
unemploymentrate <- unemploymentrate |> 
  select(Year = TIME_PERIOD, Country = geo, Class = deg_urb, dUnemployed = OBS_VALUE) |> 
  mutate(Class = recode(Class,
                           "DEG1" = "City",
                           "DEG2" = "Suburb",
                           "DEG3" = "Rural"))
...
datapt1 |>
  ggplot(
    aes(
      x = Year,
      y = Population / Population[Year == 2003] * 100,  # Relativ vekst i forhold til 2003
      color = `Region Type`
    )
  ) +
  geom_smooth(level = 0.5, span = 1) +
  geom_hline(yintercept = 100, linetype = "dashed", color = "black") +  # Horisontal linje på 100
  xlab("Year") +
  ylab("Relative Growth (2003 = 100)")
```
