...
merged <- merged |> 
  ...
  mutate(dmeanUnemployed = round((dUnemployed / sum(dUnemployed)*100), digits = 3)) |>
  ungroup()

merged <- merged |> 
  group_by(Country, Year) |> 
  mutate(dmeanEmployment = round((dEmployment / sum(dEmployment)*100), digits = 3)) |>
  ungroup()

merged <- merged |> 
  group_by(Country, Year) |> 
  mutate(dmeanHousing = round((dHousing / sum(dHousing)*100), digits = 3)) |>
  ungroup()
